...
const insumosEvento: Insumo[] = [
  { nombre: "Catering",    precio: 25, cantidad: 2 },
  { nombre: "Sonido",      precio: 80, cantidad: 1 },
  { nombre: "Decoración",  precio: 200, cantidad: 3 },
];

let total_insumos = 0;
for (const insumo of insumosEvento) {
  const subtotal = insumo.precio * insumo.cantidad;
  console.log(`${insumo.nombre}: $${subtotal}`);
  total += subtotal;
}
